...
public enum BuildingCommands {
    DROP_BUILDING("^\\s*dropbuilding\\s+(-x\\s+(?<x>\\-?\\d+)\\s*()|-t\\s+(?<type>\\w+|\".*\")\\s*()|-y\\s+" +
            "(?<y>\\-?\\d+)\\s*()){3}\\3\\5\\7$"),
    SELECT_BUILDING("^\\s*select\\s+building\\s+(-x\\s+(?<x>\\-?\\d+)\\s*()|-y\\s+" +
            "(?<y>\\-?\\d+)\\s*()){2}\\3\\5$"),
    CREATE_UNIT("^\\s*createunit\\s+(-t\\s+(?<type>\\w+|\\\".*\\s+.*\\\")\\s*()|-c\\s+" +
            "(?<count>\\-?\\d+)\\s*()){2}\\3\\5$"),
