...
public enum GameMenuCommands {
    SELECTUNIT("select\\s+unit\\s+(\\-x\\s+(?<x>[\\d]+)\\s*()|\\-y\\s+(?<y>[\\d]+)\\s*()|\\-t\\s+" +
            "(?<type>[\\S]+)()){3}\\3\\5\\7$"),

    MOVEUNIT("move\\s+unit\\s+(\\-x\\s+(?<x>[\\d]+)\\s*()|\\-y\\s+(?<y>[\\d]+)\\s*()){2}\\3\\5$"),

    PATROLUNIT("patrol\\s+unit\\s+(\\-x1\\s+(?<x1>[\\d]+)\\s*()|\\-y1\\s+(?<y1>[\\d]+)\\s*()|\\-x2\\s+" +
            "(?<x2>[\\d]+)\\s*()|\\-y2\\s+(?<y2>[\\d]+)\\s*()){4}\\3\\5\\7\\9$"),

    STOP_PATROL_UNIT("stop\\s+patrol\\s+unit(?<x>[\\d]+)\\s+(?<y>[\\d]+)\\s*"),

    SETMOODE("set\\s+(\\-x\\s+(?<x>[\\d]+)\\s*()|\\-y\\s+(?<y>[\\d]+)\\s*()|\\-s\\s+(?<moode>[\\S]+)\\s*())" +
            "{3}\\3\\5\\7$"),
    ...
    BUILD_GATE("build\\s+-q\\s+(-x\\s+(?<x>[\\d]+)\\s+(-y\\s+(?<y>[\\d]+)\\s+(?<direction>[\\S]+)\\s+" +
            "(?<gateName>.+)"),
    DISBANDUNIT("disband\\s+unit"),

    MAKE_GATE("make\\s+gate\\s+-x\\s+(?<x>[\\d]+)\\s+-y\\s+(?<y>[\\d]+)\\s+(?<direction>[\\S]+)\\s+" +
            "(?<gateName>.+)"),
   ...
   MAKE_TOWER("build\\s+-q\\s+(?<x>[\\d]+)\\s+(?<y>[\\d]+)\\s+(?<type>[\\S]+\\s+tower)"),
    ...
    MOVE_BATTERING("move\\s+battering\\s+in\\s+-x\\s+(?<x1>[\\d]+)\\s+\\s+-y\\s+(?<y1>[\\d]+)\\s+to\\s+-x\\s+" +
            "(?<x2>[\\d]+)\\s+-y\\s+(?<y2>[\\d]+)\\s*"),

    USE_BATTERING_TO_ATTACK("attack\\s+with\\s+battering\\s+-x\\s+(?<x>[\\d]+)\\s+-y\\s+(?<y>[\\d]+)\\s*"),

    MOVE_CATAPULT("move\\s+catapult\\s+in\\s+-x\\s+(?<x1>[\\d]+)\\s+\\s+-y\\s+(?<y1>[\\d]+)\\s+to\\s+-x\\s+" +
            "(?<x2>[\\d]+)\\s+-y\\s+(?<y2>[\\d]+)\\s*"),

    USE_CATAPULT_TO_ATTACK("attack\\s+with\\s+catapult\\s+-x1\\s+(?<x1>[\\d]+)\\s+-y\\s+(?<y1>[\\d]+)\\s+\\-x\\s+" +
            "(<x2>[\\d]+)\\s+-y\\s+(?<y2>[\\d]+)\\s*"),

    USE_FIXED_CATAPULT_TO_ATTACK("attack\\s+with\\s+fixed\\s+catapult\\s+-x1\\s+(?<x1>[\\d]+)\\s+-y\\s+" +
            "(?<y1>[\\d]+)\\s+\\-x\\s+(<x2>[\\d]+)\\s+-y\\s+(?<y2>[\\d]+)\\s*"),

    MOVE_FIERY_STONE_THROWER("move\\s+fiery\\s+stone\\s+in\\s+-x\\s+(?<x1>[\\d]+)\\s+\\s+-y\\s+(?<y1>[\\d]+)" +
            "\\s+to\\s+-x\\s+(?<x2>[\\d]+)\\s+-y\\s+(?<y2>[\\d]+)\\s*"),
